...
## How does server-side rendering work in the Interactivity API?

The Interactivity API allows developers to define interactive elements directly in the HTML markup using special attributes called directives. These directives describe how elements should behave and interact with the global state or local context.

Let's start with an example where we render a list of fruits using the `data-wp-each` directive.
...
Next, you define the HTML markup with the directives.
...
The same directives can also be used when using local context with a `data-wp-context` directive:
...
Once you've set up your block with `supports.interactivity`, initialized your global state and local context, and defined your HTML with directives, WordPress takes care of the rest. There's no additional code required from the developer to process these directives on the server side.

Behind the scenes, WordPress uses the `wp_interactivity_process_directives` function to find and process the directives in the HTML markup of your block. This function makes the necessary changes to the markup based on the directives and the current state or context.

As a result, the HTML sent to the browser is already in its final form, with all server-side directives processed. This means that when the page first loads in the browser, it already contains the correct initial state of all interactive elements, without needing any JavaScript to modify it.
...
This server-side processing provides several benefits:
...
### 5. Modifying the global state or local context on the client

One of the key strengths of the Interactivity API is how it bridges the gap between server-side rendering and client-side interactivity.

To do so, the global state and local context initialized on the server are serialized and made available on the client, allowing the application to continue functioning and manipulating the DOM dynamically.

Let's extend the fruit list example to include a button that adds a new fruit to the list:
...
In this example, we've added a button with a `data-wp-on--click` directive that references `actions.addFruit`. This action is defined in our JavaScript store:
...
The same example also works if instead of using global state, we are using local context:
